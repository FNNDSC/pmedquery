...
PubMed.prototype    = {
    ...
    ids_get:    async function(query) {
        ...
        const searchUrl         = `${this.baseUrl}esearch.fcgi?db=pubmed&term=${query}&api_key=${apiKey}&retmode=json`;
    ...
    ids_processasync:   async (pubmedIDs) => {
        let a_metaData = [];
    ...
    id_fetchData:   async function(id) {
        ...
        const response          = await fetch(
            `${this.baseUrl}efetch.fcgi?db=pubmed&id=${id}&retmode=xml&rettype=abstract&api_key=${this.apiKey}`
        );
